...
class Up(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, bilinear=False, fuse_mode='add'):
        super().__init__()

        self.fuse = fuse_mode
        self.in_channels = out_channels if fuse_mode == "add" else in_channels
...
def center_crop(layer, max_height, max_width):
    ...
    crop_out = layer[:, :, xy2:(xy2 + max_height), xy1:(xy1 + max_width)]
    return crop_out
