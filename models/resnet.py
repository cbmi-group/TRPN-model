...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, in_dim=3, num_classes=1000, zero_init_residual=False):
    ...
    def forward(self, x, size):
        #x 1
        x = self.conv1(x)  #1/2 1024
        x = self.bn1(x) 
        x = self.relu(x)
        C1 = self.maxpool(x) #1/4 512
        C2 = self.layer1(C1) #1/4 512
        C3 = self.layer2(C2) #1/8 256 
        C4 = self.layer3(C3) #1/16 128
        C5 = self.layer4(C4) #1/32 64
...
def resnet50(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        pre_trained = model_zoo.load_url(model_urls['resnet50'])
...
def resnet101(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        pre_trained = model_zoo.load_url(model_urls['resnet101'])
        model_dict = model.state_dict()
        pretrained_dict = {k: v for k, v in pre_trained.items() if
                            k in model_dict and v.size() == model_dict[k].size()}
        model_dict.update(pretrained_dict)
