...
class FS2_Five_Shot(nn.Module):
    def __init__(self, in_channels=1, base_dim=64, n_classes=1):
        super(FS2_Five_Shot, self).__init__()
    ...
    def forward(self, data_dict, skl=False):
        ...
        cosine_similarity_prototype_4 = 0.0
        for i in range(5):
            tmp_support_img = support_img[:,i,:,:].unsqueeze(dim=1)
            tmp_support_mask = support_mask[:,i,:,:].unsqueeze(dim=1)
            support_feats_4, _, _, _ = self.feature(tmp_support_img)
            support_prototypes_4 = torch.sum(torch.sum(support_feats_4*tmp_support_mask, dim=3), dim=2)/torch.sum(torch.sum(torch.sum(tmp_support_mask, dim=3), dim=2), dim=1, keepdim=True)
        ...
        cosine_similarity_prototype_4 = cosine_similarity_prototype_4 / 5.
...
class UNet_V1(nn.Module):
    def __init__(self, input_channels=1, base_dim=64, bilinear=False):
        ...
        self.up5 = Up(base_dim*16, base_dim*8, bilinear, fuse_mode=self.fuse_mode)
        self.up6 = Up(base_dim*8,  base_dim*4, bilinear, fuse_mode=self.fuse_mode)
        self.up7 = Up(base_dim*4,  base_dim*2, bilinear, fuse_mode=self.fuse_mode)
        self.up8 = Up(base_dim*2,  base_dim,   bilinear, fuse_mode=self.fuse_mode)
    ...
    def forward(self, x):
        ...
        o_m_8 = self.up5(x5, x4)
        o_m_7 = self.up6(o_m_8, x3)
        o_m_6 = self.up7(o_m_7, x2)
        o_m_5 = self.up8(o_m_6, x1)
