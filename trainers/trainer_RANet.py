...
class trainer_segmentation_ranet(nn.Module):
    def __init__(self, params=None):
        super(trainer_segmentation_ranet, self).__init__()
        self.args = params
        self.global_step = 0
        self.current_step = 0
    ...
    def val_one_epoch(self, epoch):
        with torch.no_grad():
            ...
            for i, data_dict in enumerate(self.val_data_loader):
                data_dict["support image raw"] = data_dict["support image"].cuda()
                data_dict["support mask raw"] = data_dict["support mask"].cuda() 
                data_dict['query image'] = data_dict['query image'].cuda()
                
                bz, shots, h, w = data_dict["support image raw"].size()
                mask_out = torch.zeros((bz,1,h,w)).cuda()
                for i in range(shots):
                    data_dict['support image'] = data_dict["support image raw"][:, i, :, :].unsqueeze(dim=1)
                    data_dict['support mask'] = data_dict["support mask raw"][:, i, :, :].unsqueeze(dim=1)
                    tmp, _, _ = self.model(data_dict, skl=self.args.val_skl)
                    mask_out += tmp
                mask_out /= shots
            ...
            support_image = data_dict['support image'].data.cpu().numpy()
            query_image = data_dict['query image'].data.cpu().numpy()
            query_mask = data_dict["query mask"].data.numpy()
